...
/* Estrutura padrAo para autOmatos */
struct automato
{
...
struct estados
{
    ...
    struct final *L; /* usamos a estutura 'final' mas isto nao é uma lista de estados finais. */
    struct estados *prox;
};
...
struct final
{
    unsigned short int value; /* Inteiro representando o nUmero de estados finalis */
    struct final *prox; /* Guia a lista */
};

/* Estrutura para o conjunto 'delta' */
struct delta
{
    unsigned short int Ei; /* Inteiro representando o estado inicial da funCAo */
    char Le[SBUFF]; /* Char representando uma letra do alfabeto */
    unsigned short int Ef; /* Inteiro representando o estado final da funCAo */
...
void automax_init(void);  /* *< OperaCOes iniciais*/
void automax_update(void); /* *< Atualiza os valores durante os ciclos*/

void auto2(void); /* Converte AFD para ER */
void auto1(void); /* Converte AFND para AFD */

void checkstring(char entrada[SBUFF]); /* Conferir se o usuArio digitou algum caractere invAlido */
void conversao_afd2er(struct automato *afd); /* Converte um AFD numa Expressao Regular */
void conversao_er2afnd(char entrada[SBUFF]); /* Converte uma Expressao Regular num AFND */

void define_State(struct delta *afndD); 
void entradainvalida(void); /* Esta funçao avisa ao usuário que a entrada de dados foi inválida e fecha o programa. */
void auto3(void); /* Converte ER para AFND */

void file_er(char sfile[SBUFF]); /* Funçao que lê um arquivo de entrada que contEm uma ExpressAo Regular */

void printAuto(struct automato *autom); /* Funçao para imprimir o autOmato. */
char alfabeto(struct delta *aux); /* FunCAo para definir o alfabeto do autOmato */
void simplificar(char expressao[SBUFF]); /* Aplica funCOes "simplificarparenteses" e "simplificarvazios"*/
const char *simplificarparenteses(char expressao[SBUFF]); /* Retira parenteses nao obrigatOrios de uma ER */
void simplificarvazios(char expressao[SBUFF]); /* Retira caracteres vazios nao obrigatOrios de uma ER */
unsigned short int checksize(struct final *lista); /* Funçao que retorna o tamanho da lista de estados finais */

int file_afd(char sfile[SBUFF]); /* lE um arquivo de entrada AFD */
int file_afnd(char sfile[SBUFF]); /* lE um arquivo de entrada AFND */

int checknovoestado(struct estados *temp, struct estados *novosestados);
struct automato *checksizeE(struct automato *afd); /* Funçao que atualiza a quantidade de estados no autOmato */
...
struct automato *novosestadosauxiliares(struct automato *afd); /* Cria dois novos estados, um de partida e um de chegada, para auxliar na exclusao dos outros */
...
struct automato *removerImpossiveis(struct automato *afnd); /* FunCAo que remove estados impossIveis de se chegar do autOmato  */
