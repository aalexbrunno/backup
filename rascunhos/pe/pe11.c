...
{
     int i, j;
    int horzP, vertP, diagULLR, diagURLL, maxP;
    ...
    for (i = 0; i < 20; i++)
    {
            for (j = 0; j <17; j++)
            {
                    horzP = grid[i][j]*grid[i][j+1]*grid[i][j+2]*grid[i][j+3];
                    if (horzP > maxP) maxP = horzP;
            }
    }
    ...
    for (i = 0; i < 17; i++)
    {
            for (j = 0; j < 20; j++)
            {
                    vertP = grid[i][j]*grid[i+1][j]*grid[i+2][j]*grid[i+3][j];
                    if (vertP > maxP) maxP = vertP;
            }
    }

    // diagonal superior esquerda - produto inferior direito
    for (i = 0; i < 17; i++)
    {
            for (j = 0; j < 17; j++)
            {
                    diagULLR = grid[i][j]*grid[i+1][j+1]*grid[i+2][j+2]*grid[i+3][j+3];
                    if (diagULLR > maxP) maxP = diagULLR;
            }
    }

    // diagonal superior direita - produto inferior esquerdo
    for (i = 0; i <17; i++)
    {
            for (j = 3; j < 20; j++)
            {
                    diagURLL = grid[i][j]*grid[i+1][j-1]*grid[i+2][j-2]*grid[i+3][j-3];
                    if (diagURLL > maxP) maxP = diagURLL;
            }
    }
