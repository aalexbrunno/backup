    ...
    while((opt = getopt(argc, argv, "vhcf:nde")) != EOF)
        switch(opt)
        {
            ...
            case 'n':
                if(argv[2] != NULL)
                {
                    /* Foi acionada a flag -f. argv[3] deve conter o nome do arquivo que vai ser lido. */
                   if(argv[3] == NULL)
                   {
                      help();
                      return EXIT_FAILURE;
...
{
    printf("# # # [ AFND -> AFD ] # # #\n");
    printf("# Digite os dados de uma AFND conforme indicado.\n");
    printf("# Ao finalizar algum dos campos que oferecem multiplas opcoes de entrada,\n# Aperte 'ENTER' com uma linha vazia para continuar \n");
...
void checkstring(char entrada[SBUFF])
{
    ...
    while(entrada[i] > 32)
    {
        if((entrada[i] < 97) || (entrada[i] > 122)) /* Caso nao seja uma letra minuscula */
           if((entrada[i] != 124) && (entrada[i] != 40) && (entrada[i] != 41) && (entrada[i] != 42) && (entrada[i] != 46)) /* Se nao forem operadores  ( ) | * . */
              if(entrada[i] != 69) /* se nao for caractere vazio 'E' */
              {
                 printf("\n O caractere \"%c\" (ASCII Dec %d) digitado eh invalido neste programa, voce pode proseguir .\n", entrada[i], entrada[i]);
                 exit(-1);
              }
              i++;
    }
