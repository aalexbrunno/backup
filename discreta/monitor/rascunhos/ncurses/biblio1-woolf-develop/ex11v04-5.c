...
#include "ex11v04-5.h" /* To be created for this template if needed */
#include <assert.h>
#include <form.h>
#include <ctype.h>
file_entry str;
		...
		case KEY_F(3):
            ...
            { 
                ...
                {  
                    ...
                    //mvwprintw(pw2->w[pw2->wi],yMAX-(yMAX-12),xMAX-(xMAX-1),"%i",flag);
                }
...
void printFormLine(wins_t *pw2, FIELD *fields[], FORM *form, livro_t *pw)
{
	...
	assert(form != NULL);
	set_form_win(form, pw2->w[pw2->wi]);
	set_form_sub(form, derwin(pw2->w[pw2->wi], 1, 30, (yMAX/2)+5, (xMAX-((xMAX/2)+3)))); //controla altura e largura da subjanela
	post_form(form);
...
void print(list *head,wins_t *pw)
{
    ...
    while(pl!=NULL)
    {
        mvwprintw(pw->w[pw->wi],(yMAX-(yMAX-15))+i,(xMAX-(xMAX-1)),"%s",pl->data);
        printf("%s", pl->data);
        pl=pl->prox;
        i++;
    }
...
void printTemplate(wins_t *pw)
{
    ...
    switch(pw->wi)
    {
        ...
        case 8:
            ...
            mvwhline(pw->w[pw->wi],(yMAX-(yMAX-2)),xMAX-(xMAX-1),0,xMAX-2);
            mvwprintw(pw->w[pw->wi],(yMAX-(yMAX-1)),(xMAX/3)+2,"== Sistema de bibioteca Smith ==");
            mvwprintw(pw->w[pw->wi],yMAX-(yMAX-3),2,"Tela 0%i: Empréstimo de Livros",pw->wi +1);
            mvwhline(pw->w[pw->wi],(yMAX-(yMAX-4)),xMAX-(xMAX-1),0,xMAX-2);
            mvwhline(pw->w[pw->wi],(yMAX-(yMAX-17)),xMAX-(xMAX-1),0,xMAX-2);
