...
setPossivelSeguro(L) :-
    retractall(possivelSeguro(_)),
    asserta(possivelSeguro(L)).

setPossivelSeguroBuraco(L) :-
    retractall(possivelSeguroBuraco(_)),
    asserta(possivelSeguroBuraco(L)).

setPossivelSeguroBat(L) :-
    retractall(possivelSeguroBat(_)),
    asserta(possivelSeguroBat(L)).
...
definePos(Lp, P, R) :-
    P == [],
    R = Lp,
    setPossivelSeguroBuraco(Lp),!.

definePos(Lp, P, R) :-
    length(P, N),
    N == 2,
    intersection(Lp,P,R),
    possivelSeguroBuraco(X),
    subtract(X, Lp, Ns),
    seguras(Ls),
    union(Ns,Ls,Nls),
    setSeguras(Nls),!.
...
/*
proximaCasa(Andarei,_):-
    andarei(Andarei),
    estou(E),
    nth1(1,Andarei,Pos),
    E == Pos,!.

proximaCasa(Andarei,Casa):-
    andarei(Andarei),
    nth1(1,Andarei,Pos),
    Casa = Pos.
*/
...
sentir_wumpus([yes,_,_,_,_,_,_],A):-
   /* (getNearHouse(NearH),
    ...
    proximaCasa(PCasa),
    E == PCasa,
    getNearHouse(New_house),
    setProximaCasa(New_house),
    proximaCasa(N),
    vaipara(N,A),!).*/
    andarei(Andarei),
    nth1(1,Andarei,Casa),
...
sentir_nada([no,no,_,no,no,no,_], A):-  
    ...
    vaipara(N,A).
/*
sentir_nada([no,no,_,no,no,no,_], goforward):-
    %gotTheGold(D),
    % D == 0,
    ondeEstou([X,Y]),
    andei(An),
...
/*separe_action([no,no,_,no,no,no,_], A) :-
    andarei(Andarei),
    %proximaCasa(Andarei,Casa),
    nth1(1,Andarei,Casa),
    vaipara(Casa,A),!.
...
apontarpara([Xqualquer,Yqualquer],turnleft) :- %inclusao do orientation
    estou([X,Y]),
    agent_orientation(O),
    O =:= 0,
...
getNearHouse(NearH) :-
    ...
    %seMoveu(X,Y,O2,CasaAnterior),
    %[XAnt,YAnt] = CasaAnterior,
    adjacentes(X,Y,Ladj),
    andarei(Landarei),
